/*
  Challenge: Given an array of nums, determine whether a specific num is present in the array.
  
  1st: iterative solution
  input: array
  output: boolean (indicating if value is present or not)
  
  2nd recursive solution 
    -  The function takes in three params, an arr, a counter, and the target val

  Pseudocode:
  - The array I'm given holds integers at each index
  - In this case, I use a for of loop to iterate through arr
    - if num === val, then I can return true
*/
...
console.log(findNum([1, 2, 3, 4, 5, 5, 6, 7, 8, 8], 6));

const findNumRecursive = function(arr, counter, target){
 ...
 if(arr[counter] === target) {
  return true;  
   ...
   counter++;
   return findNumRecursive(arr, counter, target);
 }
}
console.log(findNumRecursive([1, 2, 3, 4, 5, 5, 6, 7, 8, 8],0, 10));
